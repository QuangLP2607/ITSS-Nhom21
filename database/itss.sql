...
CREATE TABLE IF NOT EXISTS ExpiryAlerts (
    ...
    userid INT,
    status VARCHAR(50),
    FOREIGN KEY (fridgeitemid) REFERENCES fridgeitems(fridgeitemid) ON DELETE CASCADE,
    FOREIGN KEY (userid) REFERENCES users(userid)
);

---------------------------------------------------
--|| Thông báo thực phẩm hết hạn trong tủ lạnh ||--
---------------------------------------------------
...
BEGIN
    ...
    LOOP
        ...
        VALUES (NEW.fridgeitemid, NEW.expirydate, NEW.groupid, user_rec.userid, 'pending');
    END LOOP;
...
------
CREATE OR REPLACE FUNCTION update_expiry_alert()
    RETURNS TRIGGER AS $$
BEGIN
    UPDATE ExpiryAlerts
    SET alertdate = NEW.expirydate 
    WHERE fridgeitemid = NEW.fridgeitemid;

    RETURN NEW;
END;
$$ LANGUAGE plpgsql;

CREATE TRIGGER update_expiry_alert_trigger
    AFTER UPDATE OF expirydate ON fridgeitems
    FOR EACH ROW
    EXECUTE FUNCTION update_expiry_alert();
